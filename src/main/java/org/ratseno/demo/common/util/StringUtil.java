...
public class StringUtil {
     ...
     * @구현요망
     */
    public static boolean maxVal(double data, double maxValue) {
     ...
     * 계좌번호 뒷 4자리 마스킹
     */
    public static String maskForAccNo(String planValue){
        if( null == planValue ){
            return planValue;
        }

        planValue = planValue.trim();
        int strLen = planValue.length();
        int endLen = 4;
        if( endLen > strLen ) {
            return planValue;
        }
        return replaceMask(planValue, (strLen - endLen), strLen);
    }

    /*
     * 마스킹처리
     */
    public static String replaceMask(String inData, int startIdx, int endIdx ) {
        inData = inData.trim();
        int strLen = inData.length();
        StringBuffer buff = new StringBuffer();
        for (int i = 0; i < strLen; i++) {
            if( startIdx <= i && i < endIdx ) {
                buff.append("*");
            } else {
                buff.append(inData.charAt(i));
            }
        }
     ...
     * @return paramStr
     */
    public static String base64Decoding(String paramStr){
        byte[] decoded = Base64.decodeBase64(paramStr);
        paramStr = new String(decoded);
        return paramStr;
    }
     ...
     * @return paramStr
     */
    public static String base64Incoding(String paramStr){
        byte[] incoded = Base64.encodeBase64(paramStr.getBytes());
        paramStr = new String(incoded);
        return paramStr;
    }
     ...
     * @Method description :
     * @param              :
     * @return             : true : 모바일, false : 엡 or 태블릿
     * @throws             : Exception
     */
    public static boolean isMobile(HttpServletRequest req){
        ...
        for (int i = 0; i < browser.length; i++) {
            if(userAgent.toLowerCase().matches(".*"+browser[i]+".*")){
                flag = true;
            }
        }
    ...
    public static String maskedPhoneNum(String phoneNum) {
        if(null != phoneNum && phoneNum.length() > 6){
            String preNum = phoneNum.substring(0,4);
            String centerNum = "**";
            String postNum = phoneNum.substring(6,phoneNum.length());
            return preNum + centerNum + postNum;
        }else{
            return phoneNum;
        }
    }
    ...
    /**
     *<pre>
     * 인자로 받은 String이 문자열 그대로 리턴한다.
     * "-" 가 들어 있는경우 "-" 제외하고 입력받은 문자가 10자리 , 11자리인 경우 중간에 "-" 넣어서 리턴
     * 그외의 경우 문자열 그대로 리턴한다.
     *  
     *</pre>
     */
    public static String phoneConvertMinus(String phone) {

    	if (isEmpty(phone) || phone.length() <10 ) {
    		return phone;
        }
    	String tmpPhone = removeMinusChar(phone);
 	   if(tmpPhone.length() == 11){
 		   phone = tmpPhone.substring(0, 3)+"-"+tmpPhone.substring(3, 7)+"-"+tmpPhone.substring(7, 11);
 		   return phone;
 	   } else if(tmpPhone.length() == 10){
 		   phone = tmpPhone.substring(0, 3)+"-"+tmpPhone.substring(3, 6)+"-"+tmpPhone.substring(6, 10);
 		   return phone;
 	   } else {
 		   return phone; 	
 	   }
